...
public class GameBoard {
    ...
    public GameBoard(Database database) {
    ...
    // automatically create the database if db not found
    ...
    // check if the game is existed
    // under the same username with same game name
    public boolean isExistedGame(String username, String gameName){
        //check database here
        if(isExistedUser(username)){
            List<GameRecord> records = database.getAllRecords();
    ...
    // only valid username could complete the process
    // if game name (under the same username) is used, the game serialisation will be updated
    public void saveGame(String username,String gameName, String serialisation){
        if(isExistedUser(username)){
            if(isExistedGame(username,gameName)){
                database.updateGame(username,gameName,serialisation);
            }else{
                database.addGameFromQuestionableSource(username,gameName,serialisation);
            }
    ...
    // get all records
    // return a list including all game records
    public List<GameRecord> getRecords(String username){
        List<GameRecord> records = new ArrayList<>();
        List<GameRecord> gameRecords = database.getAllRecords();
        for(GameRecord record:gameRecords){
            if(record.getUsername().equals(username)){
                records.add(new GameRecord(record.getUsername(),record.getGameName(),record.getSerialisation()));
            }
        }
        return records;
    }

    // sign up a new user and save into the user database
    public void claimUser(String username){
        database.addUserFromQuestionableSource(username);
    }
